    ...
    result.price = function(args, cb) {
      var func = function(err) {
        ...
        if(!('volume' in args) ||
          ...
          cb("Missing/invalid 'volume' parameter", null);
          return;
        }
        nargs.modelVolume = args.volume;
        
        if(!('material' in args)) {
          cb("Missing 'material' parameter", null);
          return;
        }
        var mat_id = getMaterialId(args.material);
        if(mat_id < 0) {
          cb("Invalid 'material' parameter, must be either a string, number or object", null);
          return;
        }
      ...
      var func = function(err) {
        ...
        models.read_model(args, function(err, sw_model) {
          ...
          if('materials' in args) {
            nargs.mode.materials = flattenMaterials(args.materials);
          }
