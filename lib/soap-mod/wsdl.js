    ...
    for (var key in params) {
        if (key != nsAttrName) {
            ...
            if(typeof value==='object') {
              this.objectToXML(value, null, null, null, parts);
            } else {
...
WSDL.prototype.objectToXML = function(obj, name, namespace, xmlns, parts) {
    var self = this,
    ...
    if (Array.isArray(obj)) {
        for (var i=0, item; item=obj[i]; i++) {
            ...
            self.objectToXML(item, name, namespace, xmlns, parts);
        }
    }
    else if (typeof obj === 'object') {
    
        //Special case:  Need this to avoid doing stupid copies of giant 3D models
        if(obj instanceof Buffer) {
          parts.push(obj.toString('base64'));
          ...
          for (var name in obj) {
              var child = obj[name];
              parts.push(['<',ns,name,xmlnsAttrib,'>'].join(''));
              self.objectToXML(child, name, namespace, xmlns, parts);
              parts.push(['</',ns,name,'>'].join(''));
          }
        }
    }
